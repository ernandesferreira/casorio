...
(function($) {
	...
	function sb_get_map_type_control_style(string) {
	...
	/* Zoom Control Style */
	function sb_get_zoom_control_style(string) {
	...
	function sb_get_map_control_position(string) {
	...
	/* Map Type */
	function sb_get_map_type(string) {
	...
	/* Map Styles */
	function sb_get_map_style(string) {
		var mapstyle;
	...
	function sb_get_marker_animation(string) {
		var animation;
		string = string.toUpperCase();
	...
	*/
	...
	function sb_render_map( $this ) {
	 
		var $markers = $this.find('.sb-marker');
		var map_id = $this.attr('map-id');
		var zoom = parseInt($this.attr('zoom'));
		var maptypeid = sb_get_map_type($this.attr('maptype'));
		var pancontrol = sb_check_bool($this.attr('pancontrol'));
		var pancontrol_position = sb_get_map_control_position($this.attr('pancontrol_position'));
		var zoomcontrol = sb_check_bool($this.attr('zoomcontrol'));
		var zoomcontrol_position = sb_get_map_control_position($this.attr('zoomcontrol_position'));
		var zoomcontrolstyle = sb_get_zoom_control_style($this.attr('zoomcontrolstyle'));
		var maptypecontrol = sb_check_bool($this.attr('maptypecontrol'));
		var maptypecontrol_position = sb_get_map_control_position($this.attr('maptypecontrol_position'));
		var streetviewcontrol = sb_check_bool($this.attr('streetviewcontrol'));
		var streetviewcontrol_position = sb_get_map_control_position($this.attr('streetviewcontrol_position'));
		var overviewmapcontrol = sb_check_bool($this.attr('overviewmapcontrol'));
		var overviewmapcontrolvisible = sb_check_bool($this.attr('overviewmapcontrolvisible'));
		var maptypecontrolstyle = sb_get_map_type_control_style($this.attr('maptypecontrolstyle'));
		var mapstyle = sb_get_map_style($this.attr('mapstyle'));
		var weather = sb_check_bool($this.attr('weather'));
		var traffic = sb_check_bool($this.attr('traffic'));
		var transit = sb_check_bool($this.attr('transit'));
		var bicycle = sb_check_bool($this.attr('bicycle'));
		var panoramio = sb_check_bool($this.attr('panoramio'));
		var draggable = sb_check_bool($this.attr('draggable'));
		var scrollwheel = sb_check_bool($this.attr('scrollwheel'));
		var cplatitude = $this.attr('cplatitude');
		var cplongitude = $this.attr('cplongitude');
		var scalecontrol = sb_check_bool($this.attr('scalecontrol'));
		var searchtype = $this.attr('searchtype').toLowerCase();
		var searchradius = parseInt($this.attr('searchradius'));
		var searchiconanimation = sb_get_marker_animation($this.attr('searchiconanimation'));
		var searchdirectiontext = $this.attr('searchdirectiontext');
		var reloadonresize = sb_check_bool($this.attr('reloadonresize'));
			...
			service.nearbySearch(searchrequest, function(results, status) {
				if (status == google.maps.places.PlacesServiceStatus.OK) {
					for (var i = 0; i < results.length; i++) {
						sb_create_place_marker(results[i], searchiconanimation, searchdirectiontext);
					}
				}
			});
		}
	...
	*  @return	n/a
	*/
	function sb_create_place_marker(place, searchiconanimation, searchdirectiontext) {
	...
	*/
	...
	function sb_add_marker( $marker, map, map_id ) {
	 
		var latlng = new google.maps.LatLng( $marker.attr('data-lat'), $marker.attr('data-lng') );
		var animation = sb_get_marker_animation($marker.attr('animation'));
		// create marker
		var marker = new google.maps.Marker({
			position	: latlng,
			map			: map,
		...
		if($marker.html()) {
			...
			var infowindow_popup = sb_check_bool($marker.attr('infowindow'));
			if(infowindow_popup) {
				infowindow.open( map, marker );
			}
		}
	...
	*/
	...
	$(document).ready(function(){
		$('.sb-map').each(function(){
			var map_id = $(this).attr('map-id');
			sb_markers[map_id] = new Array();
			sb_render_map( $(this) );
		});
	});
...
function sb_open_marker(map_id, marker_id) {
	try {
		google.maps.event.trigger(sb_markers[map_id][parseInt(marker_id)-1], 'click');
	} catch(error) {
